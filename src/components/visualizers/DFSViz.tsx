...
type Step = { visited: string[]; stack: string[]; current: string | null; action: string };
...
function build(graph: Graph, start: string): Step[] {
  const steps: Step[] = [];
  if (!graph.adj[start]) {
    return [{ visited: [], stack: [], current: null, action: `Node "${start}" not in graph.` }];
  }
  ...
  while (stack.length) {
    ...
    const ns = [...graph.adj[cur]].reverse();
    for (const n of ns) {
      if (!visited.includes(n)) stack.push(n);
    }
    if (graph.adj[cur].length) {
      steps.push({
        visited: [...visited], stack: [...stack], current: cur,
  ...
  steps.push({ visited: [...visited], stack: [], current: null, action: "Stack empty → DFS complete." });
  return steps;
}
        ...
        render={(s) => (
          <div className="flex flex-col items-center gap-6">
            <div className="text-xs uppercase tracking-wider text-muted-foreground">
              Depth-first traversal from <span className="font-mono text-foreground">{start}</span>
            </div>
